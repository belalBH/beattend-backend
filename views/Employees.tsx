...
export const EmployeesView = ({ isDarkMode, lang }: { isDarkMode: boolean, lang: 'ar' | 'en' }) => {
  ...
  if (isAddingNew) {
    return <EmployeeDetailView onBack={(shouldRefresh) => { setIsAddingNew(false); if (shouldRefresh) fetchEmployees(); }} lang={lang} />;
  }
  ...
  const extendedEmployees = employees.map((emp, idx) => ({
    ...
    joiningDate: emp.joiningDate || (lang === 'ar' ? '20 مارس 2015' : 'Mar 20, 2015'),
    ...
    workType: emp.workType || (idx % 3 === 0 ? 'WFO' : 'WFH'),
    location: emp.location || (lang === 'ar' ? 'الرياض، السعودية' : 'Riyadh, KSA'),
    status: emp.status === 'نشط' || emp.status === 'active' ? t.active : t.inactive
  }));

  const filteredEmployees = extendedEmployees.filter(emp => 
    emp.name.toLowerCase().includes(searchTerm.toLowerCase()) || emp.id_str.includes(searchTerm)
  );
      ...
      <div className="bg-white border border-gray-50 rounded-[2rem] p-6 shadow-sm space-y-6">
        ...
        {viewMode === 'list' ? (
                ...
                {filteredEmployees.map((emp) => (
                  <tr key={emp.id} className="hover:bg-gray-50/50 transition-colors">
                    ...
                    <td className="px-4 py-4 text-center">
                      <button onClick={() => setIsAddingNew(true)} className="p-1.5 hover:bg-white rounded-lg text-gray-300 hover:text-[#15385E]"><Eye size={16} /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pt-4">
            {filteredEmployees.map((emp) => (
              <div key={emp.id} className="bg-white border border-gray-100 rounded-[2rem] p-6 shadow-sm relative">
                <button className={`absolute top-6 ${lang === 'ar' ? 'left-6' : 'right-6'} text-gray-300`}><MoreHorizontal size={20} /></button>
                ...
                <div className="flex justify-center mb-4"><img src={emp.avatar} className="w-20 h-20 rounded-full border-4 border-gray-50" /></div>
                <div className="space-y-3 pt-6 border-t border-gray-50 text-[10px] font-bold">
                  <div className="flex justify-between text-gray-400"><span>{lang === 'ar' ? 'الموقع' : 'Location'}</span><span className="text-gray-900">{emp.location}</span></div>
                  <div className="flex justify-between text-gray-400"><span>{lang === 'ar' ? 'الحالة' : 'Status'}</span><span className={emp.status === t.active ? 'text-[#15385E]' : 'text-gray-400'}>{emp.status}</span></div>
                </div>
              </div>
            ))}
          </div>
