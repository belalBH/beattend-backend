-- ============================================================================
-- BeatAttend Non-Destructive Phase 1 Upgrade Script: Odoo-Style Enterprise Payroll Engine
-- File: deploy/payroll_phase1_upgrade.sql
-- Database: beattend_staging_db
-- Target Tables: 25 Complete Phase 1 Tables with Foreign Keys, Indexes, & Constraints
-- ============================================================================
...
-- 8. GOSI Configurations Table (Full Multi-Criteria Resolver)
CREATE TABLE IF NOT EXISTS `gosi_configurations` (
  ...
  `status` ENUM('active', 'inactive') DEFAULT 'active',
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  KEY `idx_gosi_resolver` (`tenant_id`, `nationality_scope`, `contributor_category`, `insurance_system_code`, `status`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
...
CREATE TABLE IF NOT EXISTS `payslips` (
  ...
  `negative_net_status` ENUM('normal', 'require_approval', 'blocked', 'carried_forward') DEFAULT 'normal',
